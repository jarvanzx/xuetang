<template>
  <view class="page-sub-free safe-area">
    ...
    <view class="swiper-wrap">
      <swiper :indicator-dots="true" indicator-color="rgba(255,255,255,0.5)" indicator-active-color="#fff"
        ...
        <swiper-item v-for="item in bannerList" :key="item.id">
          <image class="swiper-item" :src="item.thumb_image" mode="aspectFill" @click="handleJump(item)"></image>
        </swiper-item>
      </swiper>
    </view>
    <scroll-view class="free-list bg-white" scroll-y="true" @scrolltolower="scrollLower">
      ...
      <CourseItem v-for="item in courselist" :teacher="item.teacher" :item="item" :key="item.id" :show-line="true">
      </CourseItem>
    </scroll-view>
  </view>
</template>
