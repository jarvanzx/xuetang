  ...
  <view>
    <view class="VerticalBox">
      ...
      <scroll-view class="VerticalMain" scroll-y scroll-with-animation style="height:calc(100vh )"
        ...
        <view class="course-book-item" type="hao" v-for="(item, index) in booklist" :key="index" :id="'main-' + index">
  ...
  export default {
    ...
    methods: {
      onDetail(course) {
        ...
        if (type === 1) {
          uni.navigateTo({
            url: `/pages/courseDetail/courseDetail?chapter_id=${id}&chapter=${str}`
          })
        } else {
          uni.navigateTo({
